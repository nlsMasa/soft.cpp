...
{
    Lazy<std::shared_ptr<int>> l( []() { return std::make_shared<int>( 1 ); } );
    const auto& s = l.get();
    BOOST_CHECK( s );
...
{
    bool called = false;
    Lazy<std::shared_ptr<int>, void> l( [&]() { called = true; } );
    const auto& s = l.get();
    BOOST_CHECK( !s );
    BOOST_CHECK( called );
}
...
{
    Lazy<std::shared_ptr<int>> a( [] { return std::make_shared<int>( 1 ); } );
...
{
    Lazy<std::unique_ptr<int>> l( []() { return std::make_unique<int>( 1 ); } );
    const auto& s = l.get();
    BOOST_CHECK( s );
    BOOST_CHECK_EQUAL( *s, 1 );
...
{
    bool called = false;
    Lazy<std::unique_ptr<int>, void> l( [&]() { called = true; } );
    const auto& s = l.get();
    BOOST_CHECK( !s );
    BOOST_CHECK( called );
}
