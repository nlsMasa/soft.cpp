...
BOOST_AUTO_TEST_CASE( CopyAll_Attributes )
{
    ...
    auto list = std::make_shared<ImmutableArrayEList<std::shared_ptr<EObject>>>(
        std::initializer_list<std::shared_ptr<EObject>>{eObject1, eObject2} );
...
BOOST_AUTO_TEST_CASE( Copy_Proxy )
{
    // create a package with a 2 class with one referenced in the other
    auto ecoreFactory = EcoreFactory::eInstance();
    auto ecorePackage = EcorePackage::eInstance();
    ...
    // create a model from this package
    auto eObject = eFactory->create( eClass );
    eObject->getInternal().eSetProxyURI( URI( "./test" ) );

    // check that deep copy is deep equal
    auto eObjectCopy = EcoreUtils::copy( eObject );
    BOOST_CHECK( EcoreUtils::equals( eObject, eObjectCopy ) );

    eObject->getInternal().eSetProxyURI( URI( "./test2" ) );
    BOOST_CHECK( !EcoreUtils::equals( eObject, eObjectCopy ) );
}

BOOST_AUTO_TEST_CASE( Copy_Null )
{
    BOOST_CHECK( EcoreUtils::copy( nullptr ) == nullptr );
}

BOOST_AUTO_TEST_CASE( Equals_Null )
{
    BOOST_CHECK( EcoreUtils::equals( std::shared_ptr<EObject>(), nullptr ) );
    BOOST_CHECK( !EcoreUtils::equals( std::make_shared<MockEObject>(), nullptr ) );
    BOOST_CHECK( !EcoreUtils::equals( nullptr, std::make_shared<MockEObject>() ) );
}

BOOST_AUTO_TEST_CASE( Equals_Proxy )
{
    auto obj1 = std::make_shared<MockEObject>();
    auto mockInternal1 = std::make_shared<MockEObjectInternal>();
    MOCK_EXPECT( obj1->getInternal ).returns( *mockInternal1 );
    auto obj2 = std::make_shared<MockEObject>();
    auto mockInternal2 = std::make_shared<MockEObjectInternal>();
    MOCK_EXPECT( obj2->getInternal ).returns( *mockInternal2 );

    MOCK_EXPECT( obj1->eIsProxy ).once().returns( true );
    MOCK_EXPECT( mockInternal1->eProxyURI ).once().returns( URI("./test") );
    MOCK_EXPECT( mockInternal2->eProxyURI ).once().returns( URI( "./test" ) );
    BOOST_CHECK( EcoreUtils::equals( obj1, obj2 ) );

	MOCK_EXPECT( obj1->eIsProxy ).once().returns( true );
    MOCK_EXPECT( mockInternal1->eProxyURI ).once().returns( URI( "./test1" ) );
    MOCK_EXPECT( mockInternal2->eProxyURI ).once().returns( URI( "./test2" ) );
    BOOST_CHECK( !EcoreUtils::equals( obj1, obj2 ) );

	MOCK_EXPECT( obj1->eIsProxy ).once().returns( true );
    MOCK_EXPECT( mockInternal1->eProxyURI ).once().returns( URI( "./test1" ) );
    MOCK_EXPECT( mockInternal2->eProxyURI ).once().returns( URI() );
    BOOST_CHECK( !EcoreUtils::equals( obj1, obj2 ) );

    MOCK_EXPECT( obj1->eIsProxy ).once().returns( false );
    MOCK_EXPECT( obj2->eIsProxy ).once().returns( true );
    BOOST_CHECK( !EcoreUtils::equals( obj1, obj2 ) );

}

BOOST_AUTO_TEST_CASE( Equals_EClass )
{
    auto obj1 = std::make_shared<MockEObject>();
    auto obj2 = std::make_shared<MockEObject>();
    MOCK_EXPECT( obj1->eIsProxy ).once().returns( false );
    MOCK_EXPECT( obj2->eIsProxy ).once().returns( false );

    auto class1 = std::make_shared<MockEClass>();
    auto class2 = std::make_shared<MockEClass>();
    MOCK_EXPECT( obj1->eClass ).once().returns( class1 );
    MOCK_EXPECT( obj2->eClass ).once().returns( class2 );
    BOOST_CHECK( !EcoreUtils::equals( obj1, obj2 ) );
}
